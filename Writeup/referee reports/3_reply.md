...
We do not currently know how it will be possible to accommodate the referee's objections --- that our data model has too few degrees of freedeom --- while still satisfying existing best practices in statistical analysis, which (as far as we understand) instead explicitly penalise having too many degrees of freedom in a fitted model.

To simplify subsequent discussion, let us consider the referee's own chosen illustrative example of fitting two modes to a single peak in the power spectrum. In such a case, the underlying cause of any degeneracy in the posterior distributions --- resulting in strong posterior correlations between the positions, widths, and amplitudes of the two fitted modes --- is that there are too many degrees of freedom to be fitted against a single peak in the power spectrum in a well-posed fashion (i.e. the fit is underdetermined), rather than too few. All of the posterior distributions for the positions, widths, and heights of these two degenerate modes will then deterministically overestimate the true uncertainty in those properties of that single peak (compared to, say, if only a single mode were to have been fitted to the single peak in the data). This is easily generalisable to more complicated underdetermined configurations.

If indeterminacy of this kind should result from fitting individual modes to our power spectrum, then it does not make sense to report the uncertainties from such an underdetermined model anyway, rather than applying the standard remedy of using a more parsimonious model with fewer free parameters, as we have already done. Conversely, even if our power spectrum already does not fully determine all of the parameters of our existing power-spectral model to begin with, *increasing* the number of degrees of freedom in the model, to comply with the referee's objections, can be seen to be an incorrect remedy. Further, if our existing fit to the power spectrum is already the result of purely artificial separation of single peaks into multiple components (as the referee claims), then it follows that our currently reported uncertainties are already *larger* than we would obtain with a properly-constrained model for the power spectrum, free of degeneracies, rather than being underestimates as the referee suggests below.
...
We now make clear in the text that the vertical line indicates the currently observed $\nu_\text{max}$, at which we impose in our simulations a rotation rate of 100 d. We have also rewritten our description of the figure in an effort to improve clarity.
