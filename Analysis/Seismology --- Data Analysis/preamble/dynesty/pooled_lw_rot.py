...
N_shared = 7 # 7 shared parameters: inclination, Γ, granulation (x2), background, and l=1,2 rotational splitting
shared_quantities = ['i', 'Γ', 'BG1', 'BG2', 'BG3', 'δν1', 'δν2']
...
checkpoint = "checkpoints/pooled_lw_rot.npy"
...
def prior_transform(x):
    ...
    δν1 = δν_min + (δν_max - δν_min) * x[5]
    δν2 = δν_min + (δν_max - δν_min) * x[6]
...
def chunk_model(ν, θ, only_l=-1, only_chunk=None, without=-1):
    ...
    for i in range(len(ν0)):
        ...
        if l == 0:
            ...
            ps += modes[l](ν, ν0[i], A[i], Γ, δν1, i0)
        elif l == 2:
            ps += modes[l](ν, ν0[i], A[i], Γ, δν2, i0)
