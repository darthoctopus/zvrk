...
\begin{figure*}[tb]
    ...
    \caption{Posterior distributions for pooled mode properties returned from peakbagging procedure. \remarkJO{WRITE ME}}
    \label{fig:shared}
\end{figure*}
...
The uncertainties on the stellar mass and radius that we have derived from the direct method are large, both owing to the fact that $\numax$ is difficult to measure for evolved stars with comparatively few visible modes, as well as owing to the fact that the scaling relations themselves may not be reliable at advanced stages of evolution. However, these are still helpful in bounding the parameter space for further analysis. To refine our estimates of the stellar properties, we turn to detailed modelling of stellar structure, as constrained by our measurements of individual mode frequencies.
...
From optimisation, we obtain a best-fitting point estimate of 1.18 $M_\odot$ etc \remark{fill in after power outage ends}. We will use this best-fitting model, and its associated evolutionary track, in our subsequent analysis requiring knowledge of Zvrk's temporal evolution or interior structure. More generally, in order to report uncertainties, we treat all of the models generated over the course of performing such optimisation as a set of nonuniform samples over our parameter space, $\Theta = \{\theta_i\} = \{(M_i, Y_{0,i}, Z_{0,i}, \amlt_i, t_i)\}$. Associated with this set of samples are the values of a likelihood function defined through \cref{eq:chi2} as $\mathcal{L}(\theta_i) = \exp[-\chi_\text{tot}^2(\theta_i)/2]$, evaluated for all $\theta_i \in \Theta$. If we were to compute likelihood-weighted averages and quantiles of stellar properties with respect to $\Theta$, as is often done with grid-based modelling [e.g. @cunha_plato_2023], we would effectively be providing Bayesian posterior estimates of these properties and their uncertainties, with the prior distribution determined by the sampling function over $\Theta$. This procedure would yield estimates with respect to a uniform prior when used in grid-based modelling, where $\Theta$ is sampled uniformly (typically using a discretised coordinate mesh or Sobol sequence). In our case, however, the sampling over $\Theta$ is determined by our optimisation trajectory. In order to return posterior estimates with respect to a uniform prior over our input parameters, we must divide out this sampling function. We estimate the sampling function $s(\theta)$ over $\Theta$ using a kernel density estimator, and compute weighted quantiles with respect to modified likelihood weights $w_i \propto \mathcal L(\theta_i) / s(\theta_i)$, with normalisation chosen to set $\sum_i w_i = 1$. We report the posterior medians and $\pm 1\sigma$ quantiles for our input parameters, and Zvrk's radius, in \cref{tab:t2}.
...
\begin{figure}[htbp]
    ...
    \annotate{\includegraphics[width=.435\textwidth]{t_n6_l1.pdf}}{\node[right] at (.15, .78){\textbf{(b)}: $n_p = 6, \ell = 1$};}
    \annotate{\includegraphics[width=.435\textwidth]{t_n6_l2.pdf}}{\node[right] at (.15, .78){\textbf{(c)}: $n_p = 6, \ell = 2$};}
    \caption{Stability of p-mode rotational kernels to stellar evolution. In all figures, kernels are shown of a single mode}
    \label{fig:kernels}
\end{figure}
```
...
While rapid for a red giant, Zvrk's surface rotation remains far slower than its breakup rotation, $\Omega_\text{crit} = \sqrt{GM/R^3}$. From our seismic constraints on Zvrk's mass and radius, we find $T_\text{crit} = 2\pi/\Omega_\text{crit} \sim 12.5\ \mathrm{d}$. However, both its rotational period and this critical rotation rate would have changed significantly over the course of its evolution up the red giant branch. In particular, if we are to interpret the apparent photometric variability as spot modulations, the same magnetic field that drives this spot activity ought also to be responsible for magnetic rotational braking. In the absence of further deviations from single-star evolution, we therefore would expect Zvrk to have been rotating more quickly earlier on the red giant branch. While the breakup rotation rate $\Omega_\text{crit}$ should also have been faster at earlier times,
