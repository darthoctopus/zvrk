...
We note that this was not in the referee's original report.
...
We would appreciate if the referee could specifically describe these "many paragraphs" and why they are superfluous, as otherwise we can only guess what they mean given a lack of such description. We note that we had made exactly the same request for clarification to an essentially identical point in our last reply, with which the referee has clearly not engaged. Again, we point out that we only mention clump stars in two short sentences: one only regarding the visual appearance of the echelle diagram (rather than assigning evolutionary identification), and one specifically saying that Zvrk is not a clump star. The indicated lines are where we first classify Zvrk as either a first-ascent RG, or AGB star, and are therefore not superfluous. 

We note also that the referee is fortunate to be able to tell immediately from experience that Zvrk's seismology cannot be that of a clump star (as we point out, overriding the judgement of automated analysis tools). Other readers, and for that matter several authors of this paper, are not so well-equipped. In editing this paper, we had removed many points of discussion from Sections 2 and 4 for length, owing to feedback that we received about some expository material being obvious. It is clear from the referee's reactions to Section 4, to which we respond below, that such obviousness was clearly not universal. We therefore by the same token suspect that what the referee finds obvious and unnecessary here may also not be so to non-seismologists reading the paper. As the referee may note by the nature of Section 4, this paper is clearly not intended to be read solely by seismologists. We ask that the referee reflect on this when recommending passages for deletion.

> - The lack of relevance of Section 2.4 is not because of its content, but from the inappropriate location of this section.

We would appreciate if the referee could explain how an inappropriate position of this section would invalidate its relevance irrespective of content, as that is not clear to us.

In any case, we are not able to alternatively situate this section. As written, Section 2.4 must come after both Section 2.3, which introduces the spectra used in discussion, and Section 2.2, which introduces our ASAS-SN and custom TESS photometry. Sections 2.2 and 2.3 in turn must both also come after Section 2.1, as our analysis there relies on seismic rotation rates and $\nu_\mathrm{max}$. Thus, Section 2.4 is constrained by these logical dependencies to be placed in its current position at earliest, and would not make sense being placed in subsequent parts of the paper. Since the referee appears to think otherwise, we would appreciate explicit guidance regarding alternative locations.
...
In accordance with the referee's wishes, we have adopted the phrasing 'height-to-background ratio' instead.

> The model of the background must be given

We now describe that the background for all panels of fig. 1 were found using the running-window median filter implemented in `lightkurve`. We already specify our parameterisation of the background model used in nested sampling.
...
In any case, we refer the referee to Kamiaka et al. 2018 for a clear description of why fitting mode linewidths and heights all individually (rather than pooling across multiple modes through some model) is not necessarily the ideal thing to do when modes overlap --- doing so systematically biases the resulting estimates of not only their linewidths and heights, whose joint posterior distributions are degenerate for each mode, but also the underlying frequency separations, and other derived quantities (e.g. their fig. 3). Correspondingly, while we agree that having a single pooled linewidth across all modes will return unreliable estimates of the linewidth, the referee's observations of an overlapping configuration indicate that this is in fact preferable to letting them all vary freely in the fit, as we describe having previously attempted, given these considerations. We have added this discussion to the main text. As we already note, we do not treat the linewidth as anything but a nuisance parameter in any case.
...
Such a test is not only an entire methods paper in itself (lying outside the scope of this work), but effectively already has been done. The fitted rotation rates and inclinations of Kuszlewicz+ 2019, MNRAS, 488, 572, who model multiplets with symmetric heights as we do, were found by Gehan+ 2021 (their section 4) to produce results which were consistent with an asymmetric-height technique, in the regime of approximately symmetric rotational splittings (as we also consider here by ansatz). This work concerned dipole-mode triplets at intermediate inclinations. However, in our proposed equator-on configuration, the quadrupole modes that we encounter are also effectively triplets, so these results are also applicable here.
...
We have added the requested citation. We would appreciate if the referee could specify what context they think ought to be provided here regarding this physical mechanism, beyond the reference to the work in which it is developed, which we have already given.
