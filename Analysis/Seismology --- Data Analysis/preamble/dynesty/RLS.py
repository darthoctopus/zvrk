...
N_shared = 9 # 9 shared parameters: inclination, Γ, granulation (x2), background, and RLS parameters
# shared_quantities = ['i', 'Γ', 'BG1', 'BG2', 'BG3', 'ν_in', 'ν_out', 't_shear']
# shared_quantities = ['i', 'Γ', 'BG1', 'BG2', 'BG3', 'ν_rot', 'δν_rot', 't_shear']
shared_quantities = ['i', 'Γ', 'BG1', 'BG2', 'BG3', 'ν_rot', 'δν_rot', 't_shear', 'Δt_shear']
...
checkpoint = "checkpoints/RLS.npy"
...
def prior_transform(x):
    ...
    # ν_in = δν_min + (δν_max - δν_min) * x[5]
    # ν_out = δν_min + (δν_max - δν_min) * x[6]
    # ν_rot = (ν_in + ν_out)/2
    # δν_rot = ν_out - ν_in

    ν_rot = δν_min + (δν_max - δν_min) * x[5]
    δν_rot = -δν_max + 2*δν_max * x[6]
    ...
    # return jnp.array([i, Γ, BG1, BG2, BG3, ν_in, ν_out, t_shear, *ν0, *A])
    return jnp.array([i, Γ, BG1, BG2, BG3, ν_rot, δν_rot, t_shear, Δt_shear, *ν0, *A])

def rotational_profile(t, ν0, δν, t_shear, Δt_shear):
    # if Δt_shear != 0:
        return ν0 + δν/2 * jnp.tanh((t - t_shear)/Δt_shear)
    # return np.where(t < t_shear, ν0 - δν/2, ν0 + δν/2)

def split(ν0, δν, t_shear, Δt_shear, l, n):
    return trapz(
            KK[l][n] * rotational_profile(t[l], ν0, δν, t_shear, Δt_shear),
            eig[l].r
        )
...
def chunk_model(ν, θ, only_l=-1, only_chunk=None):
    ...
    for i in range(len(ν0)):
        ...
        if l == 0:
            ...
            ps += modes[l](ν, ν0[i], A[i], Γ, split(ν_rot, δν_rot, t_shear, Δt_shear, l, n), i0)
