...
\begin{aligned}
    ...
    &+\left(L(\theta) - L_\text{obs} \over \sigma_L\right)^2 + \chi^2_\text{seis}(\theta),
\end{aligned}
\end{equation}
...
Importantly, we note that neither $\Dnu$ nor $\numax$ enter directly into the constraints derived in this fashion: for this exercise, they affect only ancillary properties of our search for best-fitting parameters $\theta = (M, Y_0, Z_0, \amlt, t)$. In particular, we search for stellar models using the differential-evolution optimisation scheme, as implemented in @mier_yabox_2017, which requires us to supply bounds on our search space. Based on our scaling-relation estimate for the seismic mass, we restrict our attention to $M/M\odot \in [1.05, 1.35]$; we also restrict our attention to stellar models within $\pm 25\%$ of our adopted value of \numax, as a proxy for evolution up the RGB. For the remainder of our parameters, we adopt fairly large bounds of $Y_0 \in [0.248, 0.3]$, $Z_0 \in [0.002, 0.02]$, and $\amlt \in [1.5, 2.0]$. \remarkJO{check actual values once IfA power outage is over}
