...
We thank the referee for a detailed and critical report, to which points we respond in the order that they appear.

It is important that we address the fact that a substantial majority of the referee’s comments arise from an unfortunate plotting error on our part (first panel of Figure 1). The referee correctly points out that, as depicted, the seismic detection is insecure. However, the data are not correctly plotted. We think that a corrected figure will alleviate these concerns, and may render many of these comments substantively no longer applicable. We mark all comments addressing our figure-related mishap with an asterisk (*).
...
We thank the referee for pointing out this mistake. We agree that this ought not to have been done, and have deleted this part of the text.
...
We thank the referee for pointing out the rounding error in our description of the frequency resolution, which we have corrected. We have placed the apparent magnitude in the introduction.
...
Given that the SNR is sufficient to yield statistical evidence for quadrupole modes, we are not sure how the above statement follows from the previous remark. 
...
Our discussion of the T Tauri hypothesis *directly addresses* the possibility that our p-mode detection was spurious, since our identification of the star as a first-ascent red giant is based only on p-mode asteroseismology. If we were to ignore our putative detection of p-modes, the combination of fast rotation and a high lithium abundance in this range of temperature, absolute magnitude, and spectroscopic surface gravity would immediately suggest identification of the object as being a T Tauri star (which, as we note in the manuscript, had indeed accidentally been done with V501 Aur previously). As such, we believe that this section is highly relevant, as it directly connects our other, non-seismic, observational constraints on Zvrk, with the referee's own concerns that our p-mode oscillations are spurious. We have made this connection clearer in the main text.
