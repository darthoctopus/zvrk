...
\end{aligned}
\end{equation}
where $\chi^2_\text{seis}$ is a cost function incorporating constraints from the individual mode frequencies. Importantly, we note that neither $\Dnu$ nor $\numax$ enter directly into the constraints derived in this fashion: for this exercise, they only set the bounds of the optimisation scheme that we use to search for best-fitting parameters $\theta$.
...
\begin{figure}[htbp]
    ...
    \caption{Stability of p-mode rotational kernels to stellar evolution.}
    \label{fig:kernels}
\end{figure}
```
