...
To simplify discussion of this, let us consider the referee's own chosen illustrative example of fitting two modes to a single peak in the power spectrum. In such a case, the underlying cause of any degeneracy in the posterior distributions --- resulting in strong posterior correlations between the positions, widths, and amplitudes of the two fitted modes --- is that there are too many degrees of freedom to be fitted against a single peak in the power spectrum in a well-posed fashion (i.e. the fit is underdetermined), rather than too few. All of the posterior distributions for the positions, widths, and heights of these two degenerate modes will then deterministically overestimate the true uncertainty in those properties of that single peak (compared to, say, if only a single mode were to have been fitted to the single peak in the data). This is easily generalisable to more complicated underdetermined configurations.

The referee appears to believe that indeterminacy of this kind will result if we fit individual modes to our power spectrum. If so, we are then unsure why they should nonetheless appear to prefer us to report the uncertainties that would result from fitting an underdetermined model anyway, rather than applying the standard remedy of using a more parsimonious model with fewer free parameters. Conversely, if the referee believes that the power spectrum already does not intrinsically provide enough information to fully determine all of the parameters of our existing power-spectral model to begin with, then *increasing* the number of degrees of freedom in it in response is clearly an incorrect remedy. Further, if the referee believes that our existing fit to the power spectrum is already the result of purely artificial separation of single peaks into multiple components (enabled by such indeterminacy), then it follows that our currently reported uncertainties are *larger* than we would obtain with a properly-constrained model for the power spectrum, free of degeneracies, rather than being underestimates as the referee suggests below.
...
We have now performed additional tests in Appendix C comparing both of our differential-rotation scenarios with the case of solid-body rotation. This now allows us to quantitatively assess the strength of any claim of differential rotation through a likelihood-ratio test against a "null hypothesis" scenario of solid-body rotation, which we now also more faithfully characterise using the machinery of RLS inversions. Moreover, this likelihood-ratio test by construction does not depend on the width of the posterior distribution, even if this had been misestimated. While this is (as the referee describes) not conclusive, *both* scenarios are nonetheless favoured over the solid-body scenario, with better than $1\sigma$ support. In any case, even if the tests had gone the other way, a failure to reject the null hypothesis still does not entitle us to accept it and claim a detection of solid-body rotation.
...
We understand that the referee's suggestion may be motivated by the fact that all of the rest of §2 is concerned strictly with the gathering and first-order analysis of observational data. Conversely, §2.4 contains second-order analysis of the kind mostly also performed in §3. This being the case, we agree that it makes sense to split §2.4 off from the rest of §2. However, in our last reply, we had explained why, in our view, §2.4 had to come before §3.1, since all following analysis relied on the evolutionary identification discussed therein. We believe this is still true. Rather than placing it into its own section altogether (which would be very short), we have decided instead to transfer it over to the beginning of §3. Since this does not materially affect the content of the section --- merely its organisation --- we have opted not to bold out the text in the revised manuscript.
...
We now make clear in the text that the vertical line indicates the currently observed nu_max, at which we impose in our simulations a rotation rate of 100 d. We have also rewritten our description of the figure in an effort to improve clarity.
...
Since we still feel that the material presented in Section 4 remains insufficient for a standalone paper, we have instead taken up the referee's suggestion of writing up an introductory paragraph for the section, rather than opening with §4.1 without preamble.